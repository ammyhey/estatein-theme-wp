...
function estatein_image($field,$size='large',$post_id=false,$class='',$fallback_alt='') {
  $post_id = estatein_resolve_field_post_id($post_id);
...
function estatein_asset_icon($name, $class = '', $width = 24, $height = 24) {
  ...
  return '<img class="' . esc_attr($class) . '" src="' . esc_url(get_theme_file_uri($file)) . '" alt="" width="' . (int) $width . '" height="' . (int) $height . '" loading="lazy" decoding="async" aria-hidden="true">';
}
...
function estatein_kicker($label = '') {
  ...
  $html .= '<img class="kicker-icon" src="' . esc_url($src) . '" alt="" width="69" height="30" loading="lazy" decoding="async" aria-hidden="true">';
  if ($label) {
